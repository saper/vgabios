...
vesa_EDID:
...
#ifdef LOWRES
  db 0x81, 0x9A                       /* 0x0026 Standard timing #1 (1280 x 1024 @ 70Hz) */
  ...
  db 0xD1, 0xC0                       /* 0x0026 Standard timing #1 (1920 x 1080 @ 60 Hz) */
  db 0xA9, 0xC0                       /* 0x0030 Standard timing #2 (1600 x 900 @ 60 Hz) */
  db 0xA9, 0x40                       /* 0x0034 Standard timing #3 (1600 x 1200 @ 60 Hz) */
  db 0x61, 0x59                       /* 0x0028 Standard timing #4 (1024 x 768 @ 85 Hz) */
  db 0x45, 0x59                       /* 0x002A Standard timing #5 (800 x 600 @ 85 Hz) */
  db 0x81, 0xCA                       /* 0x002C Standard timing #6 (1280 x 720 @ 70 Hz) */
  db 0x81, 0x0A                       /* 0x002E Standard timing #7 (1280 x 800 @ 70 Hz) */
  db 0x01, 0x01                       /* 0x0032 Standard timing #8 (unused) */

                                      /* 0x0036 First 18-byte descriptor (1920 x 1080) */
  db 0x02, 0x3A                       /*        Pixel clock = 148500000 Hz */
  db 0x80                             /* 0x0038 Horizontal addressable pixels low byte (0x0780 & 0xFF) */
  db 0x90                             /* 0x0039 Horizontal blanking low byte (0x0290 & 0xFF) */
  db 0x72                             /* 0x003A Horizontal addressable pixels high 4 bits ((0x0780 & 0x0F00) >> 4), and */
                                      /*        Horizontal blanking high 4 bits ((0x0290 & 0x0F00 ) >> 8) as low bits */
  db 0x38                             /* 0x003B Vertical addressable pixels low byte (0x0438 & 0xFF) */
  db 0x26                             /* 0x003C Vertical blanking low byte (0x0026 & 0xFF) */
  db 0x40                             /* 0x003D Vertical addressable pixels high 4 bits ((0x0438 & 0x0F00) >> 4), and */
                                      /*        Vertical blanking high 4 bits ((0x0026 & x0F00) >> 8) */
  db 0x78                             /* 0x003E Horizontal front porch in pixels low byte (0x0078 & 0xFF) */
  db 0xD0                             /* 0x003F Horizontal sync pulse width in pixels low byte (0x00D0 & 0xFF) */
  db 0x13                             /* 0x0040 Vertical front porch in lines low 4 bits ((0x0001 & 0x0F) << 4), and */
                                      /*        Vertical sync pulse width in lines low 4 bits (0x0003 & 0x0F) */
  db 0x00                             /* 0x0041 Horizontal front porch pixels high 2 bits (0x0078 >> 8), and */
                                      /*        Horizontal sync pulse width in pixels high 2 bits (0x00D0 >> 8), and */
                                      /*        Vertical front porch in lines high 2 bits (0x0001 >> 4), and */
                                      /*        Vertical sync pulse width in lines high 2 bits (0x0003 >> 4) */
  db 0x14                             /* 0x0042 Horizontal addressable video image size in mm low 8 bits (0x0214 & 0xFF) */
  db 0x2B                             /* 0x0043 Vertical addressable video image size in mm low 8 bits (0x012B & 0xFF) */
  db 0x21                             /* 0x0044 Horizontal addressable video image size in mm high 8 bits (0x0214 >> 8), and */
                                      /*        Vertical addressable video image size in mm high 8 bits (0x012B >> 8) */
  db 0x00                             /* 0x0045 Left and right border size in pixels (0x00) */
  db 0x00                             /* 0x0046 Top and bottom border size in lines (0x00) */
  db 0x1E                             /* 0x0047 Flags (non-interlaced, no stereo, analog composite sync, sync on */
                                      /*        all three (RGB) video signals) */
